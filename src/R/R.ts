 ...
 */
 const executeShellCommand = (command: string, args: ReadonlyArray<string>) => {
	let stdout;
	let stderr;
 ...
 */
 const executeShellCommandAsync = (command: string, args: ReadonlyArray<string>) => {
 ...
 */
const isRscriptInstalled = (path?: string) => {
	var installationDir = "";
 ...
 */
export const executeRCommand = (command: string, RBinariesLocation?: string) => {
	...
	if (RscriptBinaryPath) {
		...
		if (commandResult.stdout) {
			return filterMultiline(commandResult.stdout);
		} else {
 ...
 */
export const executeRCommandAsync = (command: string, RBinariesLocation?: string) => {
	return new Promise(function(resolve, reject) {
		...
		if (RscriptBinaryPath) {
			var args = ["-e", command];
			executeShellCommandAsync(RscriptBinaryPath, args).then((out) => {
				const output = filterMultiline(out);
				resolve(output);
			}).catch((stderr) => {
 ...
 * execution output, 0 if there was an error
 */
export const executeRScript = (fileLocation: string, RBinariesLocation?: string) => {
 ...
 */
const convertParamsArray = (params: Object) => {
	var methodSyntax = ``;
 ...
 */
export const callMethod = (fileLocation: string, methodName: string, params: Object, RBinariesLocation?: string) => {
	...
	const output = executeRCommand(`source('${fileLocation}') ; print(${methodSyntax})`, RBinariesLocation);
 ...
 */
export const callMethodAsync = (fileLocation: string, methodName: string, params: string[], RBinariesLocation: string) => {
	return new Promise(function(resolve, reject) {
 ...
 * @returns {string} the execution output of the function, 0 in case of error
 */
export const callStandardMethod = (methodName: string, params: Object, RBinariesLocation: string) => {
	...
	const output = executeRCommand(`print(${methodSyntax})`, RBinariesLocation);
 ...
 */
const filterMultiline = (commandResult: string) => {
	let data: (undefined | typeof NaN | string)[];
