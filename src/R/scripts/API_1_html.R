...
run = function(extracted_concepts_input) {
  name_of_output_file <- "src/R/outputs/sample_graph.html"  

  # df <- read.csv("src/R/material/sample-syllabus-extracted-concepts.csv")

  df <- data.frame(extracted_concepts=extracted_concepts_input)
  ...
  if(length(which(wikipedia$edge==1)) < 2) {
    write("Error", stderr())
    return()
  }
