...
export const updateFlowQuery = async (
  ...
  await Promise.all(
    flow.nodes?.map(async (obj: PolyglotNode) => {
      if (
        currentFlow.nodes.filter((node: any) => node._id === obj._id).length &&
        obj.type !==
          currentFlow.nodes.filter((node: any) => node._id === obj._id)[0].type
      ) {
    ...
    Object.keys(nodes).map(async (key: string) => {
      try {
        ...
        if (result?.upsertedIds)
          nodeIds.push(
            ...Object.keys(result?.upsertedIds).map(
              (key) => result?.upsertedIds[+key],
            ),
          );
      } catch (e) {
        console.log(e);
  ...
  await Promise.all(
    flow.edges?.map(async (obj: PolyglotEdge) => {
      if (
        currentFlow.edges.filter((edge: any) => edge._id === obj._id).length &&
        obj.type !==
          currentFlow.edges.filter((edge: any) => edge._id === obj._id)[0].type
      ) {
  ...
  return await PolyglotFlowModel.findByIdAndUpdate(flow._id, flow, {
    new: true,
  });
};
