...
export const MONGO_URL = (process.env.MONGODB_URI || 'mongodb://prova:SECRET@localhost:27017') + (process.env.MONGODB_CERTIFICATE ? encodeURIComponent(`${__dirname}/certificate.pem`) : "");

export const MONGODB_CERTIFICATE = (process.env.MONGODB_CERTIFICATE) as string;

if (MONGODB_CERTIFICATE) {
    fs.writeFileSync(`${__dirname}/certificate.pem`, MONGODB_CERTIFICATE)
}
